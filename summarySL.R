##updated summary method for different types of loss
...
summary.CV.SuperLearner <- function(object, obsWeights = NULL, delta=0.5, ...) {
  ...
  } else if (method %in% c("method.mae")) {
    for (ii in seq_len(V)) {
      Risk.SL[ii] <- mean(obsWeights[folds[[ii]]] * abs(Y[folds[[ii]]] - SL.predict[folds[[ii]]]))
      Risk.dSL[ii] <- mean(obsWeights[folds[[ii]]] * abs(Y[folds[[ii]]] - discreteSL.predict[folds[[ii]]]))
      Risk.library[, ii] <- apply(library.predict[folds[[ii]], , drop = FALSE], 2, function(x) 
        mean(obsWeights[folds[[ii]]] * abs(Y[folds[[ii]]] - x)))
    }
    se <- rep.int(NA, (length(library.names) + 2))
  } else if (method %in% c("method.huber")) {
    for (ii in seq_len(V)) {
      Risk.SL[ii] <- huber_loss(Y[folds[[ii]]], SL.predict[folds[[ii]]], delta=delta)
      Risk.dSL[ii] <- huber_loss(Y[folds[[ii]]], discreteSL.predict[folds[[ii]]],
                                 delta=delta)
      Risk.library[, ii] <- apply(library.predict[folds[[ii]], , drop = FALSE], 
                                  2, function(x) 
        huber_loss(Y[folds[[ii]]], x, delta=delta))
    }
    se <- rep.int(NA, (length(library.names) + 2))
...
print.summary.CV.SuperLearner <- function(x, digits = max(2, getOption("digits") - 2), ...) {
  ...
  } else if (x$method %in% c("method.mae")) {
    cat("Mean Absolute Error")
  }else if (x$method %in% c("method.huber")) {
    cat("Huber loss")
  }else  {
